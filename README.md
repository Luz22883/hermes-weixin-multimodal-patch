...
### 3. media + text 合并规则改成客观时间规则

不再依赖模糊的“意图判断”，而是明确采用时序规则：
...
- 保证 `/retry`、`/undo`、`/compress` 都能依赖统一 schema
- 让纯文本输入和多模态输入共享同一持久化契约
...
同样恢复 surviving user rows 的 richer metadata。

---

### 7. JSONL richer transcript 优先策略

本轮**没有改 SQLite schema**。
...
1. 不做 SQLite schema migration
2. 不把 SQLite 直接升级成 richer metadata source of truth
3. 不尝试持久化完整原始平台 `raw_message`
4. 不在本轮引入新的 video transcript block 语义
5. 不声称已经覆盖 Hermes 全仓库所有集成测试

---

## 当前验证范围

本补丁主要通过 **focused regression tests** 与最小 smoke 验证确认行为。
...
1. SQLite 新增 `metadata_json`
2. 新消息开始双写
3. JSONL 继续保留一段时间
4. 逐步切换到 SQLite 为主
5. 再做 lazy migration / opportunistic backfill
