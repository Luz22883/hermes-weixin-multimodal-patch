...
class SessionStore:
    ...
    def load_transcript(self, session_id: str) -> List[Dict[str, Any]]:
        ...
        # Prefer JSONL when it is at least as complete as SQLite.
        #
        # JSONL preserves gateway-specific rich fields such as `gateway_event`
        # for multimodal retries/replay, while SQLite currently stores the core
        # conversation shape only. If JSONL has the same number of messages, use
        # it to preserve richer user-event metadata.
        if len(jsonl_messages) >= len(db_messages):
            if db_messages and len(jsonl_messages) != len(db_messages):
                logger.debug(
