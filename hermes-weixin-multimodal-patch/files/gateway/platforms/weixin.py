...
async def _send_message(
    ...
    *,
    ...
    if ret not in (0, None) or errcode not in (0, None):
        raise RuntimeError(
            f"iLink sendmsg failed ret={ret} errcode={errcode} errmsg={response.get('errmsg', '')} response={response}"
        )
    if not response:
        logger.warning(
            "weixin: sendmsg returned empty response to=%s client_id=%s context_token=%s",
            _safe_id(to),
            client_id,
            bool(context_token),
        )
        ...
        logger.info(
            "weixin: sendmsg acknowledged to=%s client_id=%s ret=%s errcode=%s",
            _safe_id(to),
            client_id,
            ret,
            errcode,
    ...
    async def send_image_file(
        ...
        **kwargs,
        ...
        if not resolved_path:
            return SendResult(success=False, error="No image path provided")
        return await self.send_document(chat_id, resolved_path, caption=caption, metadata=metadata)
