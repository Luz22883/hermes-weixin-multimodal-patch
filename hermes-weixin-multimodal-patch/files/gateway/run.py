...
def _classify_attachment_block_type(event: MessageEvent, media_type: str) -> str:
    """Classify an attachment consistently across transcript and inbound paths."""
    normalized_media_type = str(media_type or "").strip().lower()
    message_type = getattr(event, "message_type", None)

    if normalized_media_type.startswith("image/") or message_type == MessageType.PHOTO:
        return "input_image"
    if normalized_media_type.startswith("audio/") or message_type in (MessageType.VOICE, MessageType.AUDIO):
        return "input_audio"
    return "input_file"
...
class GatewayRunner:
        ...
        if _quick_key in self._running_agents:
            ...
            if _cmd_def_inner and _cmd_def_inner.name == "stop":
                ...
                if adapter and hasattr(adapter, 'get_pending_message'):
                    adapter.clear_pending_messages(_quick_key)  # discard queued follow-ups
                if _quick_key in self._running_agents:
                    del self._running_agents[_quick_key]
    ...
    @staticmethod
    def _build_structured_user_content(event: MessageEvent) -> List[Dict[str, Any]]:
        blocks: List[Dict[str, Any]] = []
        for i, path in enumerate(getattr(event, "media_urls", None) or []):
            mtype = event.media_types[i] if i < len(event.media_types) else ""
            block_type = _classify_attachment_block_type(event, mtype)
            if block_type == "input_image":
                blocks.append({"type": "input_image", "image_path": path, "media_type": mtype or "image/*"})
            elif block_type == "input_audio":
                blocks.append({"type": "input_audio", "audio_path": path, "media_type": mtype})
            else:
                blocks.append({"type": "input_file", "file_path": path, "media_type": mtype})
        if event.text:
            blocks.append({"type": "input_text", "text": event.text})
        return blocks

    def _build_user_transcript_entry(
        self,
        *,
        event: MessageEvent,
        message_text: str,
        ...
        entry: Dict[str, Any] = {"role": "user", "content": message_text, "timestamp": timestamp}
        structured = self._build_structured_user_content(event)
        if not structured:
            structured = [{"type": "input_text", "text": event.text or ""}]
        entry["gateway_event"] = {
            "original_text": event.text or "",
            "message_type": event.message_type.value if getattr(event, "message_type", None) else "text",
    ...
    def _decorate_new_messages_for_transcript(
        self,
        *,
        ...
        gateway_event = self._build_user_transcript_entry(
            event=event,
        ...
        for msg in new_messages:
            ...
            if msg.get("role") == "user" and gateway_event:
                entry["gateway_event"] = gateway_event
            decorated.append(entry)
        return decorated

    @staticmethod
    def _rehydrate_preserved_user_gateway_events(
        original_history: List[Dict[str, Any]],
        rewritten_history: List[Dict[str, Any]],
    ) -> List[Dict[str, Any]]:
        """Restore surviving user gateway_event metadata after transcript rewrites."""
        def _entry_key(entry: Dict[str, Any] | None) -> tuple[str, str] | None:
            if not entry:
                return None
            return (
                entry.get("role", ""),
                json.dumps(entry.get("content"), ensure_ascii=False, sort_keys=True),
            )

        original_candidates: dict[tuple[str, str], list[int]] = {}
        for idx, entry in enumerate(original_history or []):
            if entry.get("role") != "user" or not entry.get("gateway_event"):
                continue
            original_candidates.setdefault(_entry_key(entry), []).append(idx)

        rehydrated: List[Dict[str, Any]] = []
        used_original_indexes: set[int] = set()
        for idx, entry in enumerate(rewritten_history or []):
            restored = dict(entry)
            if restored.get("role") == "user":
                key = _entry_key(restored)
                candidate_indexes = [
                    candidate_idx
                    for candidate_idx in original_candidates.get(key, [])
                    if candidate_idx not in used_original_indexes
                ]
                if len(candidate_indexes) == 1:
                    matched_idx = candidate_indexes[0]
                else:
                    prev_key = _entry_key(rewritten_history[idx - 1]) if idx > 0 else None
                    next_key = _entry_key(rewritten_history[idx + 1]) if idx + 1 < len(rewritten_history) else None
                    anchored_both = []
                    anchored_prev = []
                    anchored_next = []
                    for candidate_idx in candidate_indexes:
                        candidate_prev = _entry_key(original_history[candidate_idx - 1]) if candidate_idx > 0 else None
                        candidate_next = _entry_key(original_history[candidate_idx + 1]) if candidate_idx + 1 < len(original_history) else None
                        if prev_key is not None and candidate_prev == prev_key:
                            anchored_prev.append(candidate_idx)
                        if next_key is not None and candidate_next == next_key:
                            anchored_next.append(candidate_idx)
                        if (
                            prev_key is not None
                            and next_key is not None
                            and candidate_prev == prev_key
                            and candidate_next == next_key
                        ):
                            anchored_both.append(candidate_idx)

                    if len(anchored_both) == 1:
                        matched_idx = anchored_both[0]
                    elif len(anchored_next) == 1:
                        matched_idx = anchored_next[0]
                    elif len(anchored_prev) == 1:
                        matched_idx = anchored_prev[0]
                    else:
                        matched_idx = None

                if matched_idx is not None:
                    restored["gateway_event"] = original_history[matched_idx]["gateway_event"]
                    used_original_indexes.add(matched_idx)
            rehydrated.append(restored)
                    ...
                    try:
                        ...
                        if _hyg_runtime.get("api_key"):
                            ...
                            if len(_hyg_msgs) >= 4:
                                ...
                                _compressed = self._rehydrate_preserved_user_gateway_events(
                                    history,
                                    _compressed,
                                )
        ...
        try:
            ...
            if not agent_failed_early:
                ...
                if not new_messages:
                    self.session_store.append_to_transcript(
                        session_entry.session_id,
                        self._build_user_transcript_entry(
                            event=event,
                            message_text=message_text,
                            timestamp=ts,
                        )
                    ...
                    for entry in self._decorate_new_messages_for_transcript(
    ...
    async def _handle_retry_command(self, event: MessageEvent) -> str:
        ...
        for i in range(len(history) - 1, -1, -1):
            if history[i].get("role") == "user":
                last_user_entry = history[i]
                last_user_msg = history[i].get("content", "")
                last_user_idx = i
                break
        
        if not last_user_msg and not last_user_entry:
        ...
        gateway_event = (last_user_entry or {}).get("gateway_event") or {}
        retry_text = gateway_event.get("original_text", last_user_msg)
        retry_message_type_raw = gateway_event.get("message_type", "text")
        try:
            retry_message_type = MessageType(retry_message_type_raw)
        except Exception:
            retry_message_type = MessageType.TEXT

        # Re-send by reconstructing the prior user event as faithfully as possible
        retry_event = MessageEvent(
            text=retry_text,
            message_type=retry_message_type,
            source=source,
            raw_message=event.raw_message,
            media_urls=list(gateway_event.get("media_urls") or []),
            media_types=list(gateway_event.get("media_types") or []),
        )
    ...
    async def _handle_undo_command(self, event: MessageEvent) -> str:
        ...
        removed_entry = history[last_user_idx]
        removed_msg = removed_entry.get("content", "")
        removed_count = len(history) - last_user_idx
        self.session_store.rewrite_transcript(session_entry.session_id, history[:last_user_idx])
        # Reset stored token count — transcript was truncated
        session_entry.last_prompt_tokens = 0
        
        preview_text = (removed_entry.get("gateway_event") or {}).get("original_text") or removed_msg
        if isinstance(preview_text, list):
            preview_text = str(preview_text)
        preview = preview_text[:40] + "..." if len(preview_text) > 40 else preview_text
    ...
    async def _handle_compress_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            compressed = self._rehydrate_preserved_user_gateway_events(history, compressed)
    ...
    async def _run_agent(
                ...
                try:
                    ...
                    if hasattr(_adapter, 'has_pending_interrupt') and _adapter.has_pending_interrupt(session_key):
                        ...
                        if agent:
                            ...
                            # path finds it.
                            _peek_event = _adapter.peek_pending_message(session_key) if hasattr(_adapter, 'peek_pending_message') else None
        ...
        try:
                ...
                while True:
                    ...
                    if not _interrupt_detected.is_set() and session_key:
                        ...
                        if (_backup_adapter and _backup_agent
                            ...
                            _bp_event = _backup_adapter.peek_pending_message(session_key) if hasattr(_backup_adapter, 'peek_pending_message') else None
            ...
            if result and adapter and session_key:
                ...
                if pending_event and interrupt_message:
                    pending_event.text = BasePlatformAdapter._merge_caption(
                        pending_event.text,
                        interrupt_message,
                    )
                if result.get("interrupted") and not pending_event and interrupt_message:
                    logger.debug(
                        "Using interrupt_message fallback without queued event for session %s",
